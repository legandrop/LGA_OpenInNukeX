...
APP_NAME="LGA OpenInNukeX"           # .app y ejecutable — SIEMPRE arranca con "LGA"
ARTIFACT_NAME="LGA_OpenInNukeX"      # base del .zip/.dmg — sin espacios
DISPLAY_NAME="LGA OpenInNukeX"       # nombre visible del volumen y del titulo

# El SUFIJO del nombre de archivo NO sigue la convencion `_Mac_v<version>` del template, y
# es a proposito: el catalogo de updates de PipeSync ya declara para esta app el patron
# `LGA_OpenInNukeX_v{v}_mac.zip` (UpdateCatalog.cpp), y su card busca el asset del release
# por ese patron. Cambiarlo dejaria a PipeSync sin encontrar la actualizacion. El DMG usa el
# mismo orden por coherencia dentro del release, aunque a el no lo consuma nadie.
ARTIFACT_SUFFIX="_mac"
# ---------------------------------------------------------------------------------------

show_help() {
    echo "Uso: $0 [--no-open]"
    echo ""
    echo "Crea deploy/${ARTIFACT_NAME}_v<version>_mac.dmg para una instalacion nueva."
    echo "El artefacto de actualizacion es ${ARTIFACT_NAME}_v<version>_mac.zip (lo arma deploy.sh --zip)."
    echo ""
...
require_cmd python3
require_cmd ditto
# `strings` lo usa la verificacion del final. Sin el, el chequeo de backgroundImageAlias da
# falso, se BORRA un DMG que estaba bien y el mensaje culpa al layout — mandando a perseguir
# un problema inexistente. Mejor cortar aca con la causa real.
require_cmd strings
...
# OJO: en esta app el archivo VERSION vive en la RAIZ del repo, no junto al CMakeLists:
# QtClient/ es solo el cliente Qt, y la raiz es tambien el plugin de Nuke que se publica.
# Sin fallback al CMakeLists a proposito. El template lo tiene porque hay apps LGA sin archivo
# VERSION, pero esta lo tiene siempre y `tools/sync_version.py` lo mantiene. El fallback aca
# seria peor que no tenerlo: el CMake declara `1.77.0` y el VERSION `1.77`, asi que produciria
# `..._v1.77.0_mac.dmg` mientras github_release_mac.sh busca `..._v1.77_mac.dmg`.
if [[ ! -f "../VERSION" ]]; then
    echo "ERROR: falta ../VERSION (vive en la raiz del repo, no junto al CMakeLists)."
    exit 1
fi
APP_VERSION="$(tr -d '\r\n' < ../VERSION)"
...
DMG_NAME="${ARTIFACT_NAME}_v${APP_VERSION}${ARTIFACT_SUFFIX}.dmg"
